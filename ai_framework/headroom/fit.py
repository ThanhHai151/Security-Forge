...
def _truncate_log(log: str, max_tokens: int) -> tuple[str, int]:
    """Truncate a log to a head+tail window sized to ~max_tokens, with a marker."""
    before = count_tokens(log)
    if before <= max_tokens:
        return log, 0
    keep_chars = max(8, max_tokens * 4)  # mirrors the token heuristic (4 chars/token)
    half = keep_chars // 2
    dropped = len(log) - 2 * half
...
def fit(request: TurnRequest, budget: Budget) -> FittedRequest:
    ...
    # --- 4. Truncate large tool logs (last resort) -----------------------------------
    if total() > limit:
        for turn in turns:
            for result in turn.tool_results:
                new_log, saved = _truncate_log(result.log, budget.max_tool_log_tokens)
                if saved:
                    result.log = new_log
                    actions.append(
                        CompactionAction(
                            kind="truncate_log",
                            detail=f"truncated log for call {result.call_id}",
                            tokens_saved=saved,
                        )
                    )
            if total() <= limit:
